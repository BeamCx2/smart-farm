        ...
        <div className="p-4 font-sans min-h-screen bg-gray-50/30">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                <table className="w-full text-sm">
                    <thead className="bg-gray-50/80 border-b border-gray-50 font-black uppercase text-[9px] text-gray-400 tracking-widest">
                        <tr>
                        ...
                        {orders.map((o) => {
                            ...
                            return (
                                <tr key={o.id} className="hover:bg-emerald-50/10 transition-colors group">
                                    ...
                                    <td className="px-6 py-7 font-black">
                                        <div className="text-gray-900 text-xs uppercase">{o.customer?.name}</div>
                                        <div className="text-[9px] text-gray-400 tracking-widest mt-1 leading-none">{o.customer?.phone}</div>
                                    </td>
                                    <td className="px-6 py-7 text-center">
                                        {/* 📍 ดึงตรงๆ จาก utils.js (label) ไม่ต้องแต่งเติมในโค้ดนี้ครับ */}
            ...
            {selectedOrder && (
                <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-md flex items-center justify-center p-4 z-[999]">
                    <div className="bg-white rounded-[3.5rem] p-10 max-w-5xl w-full max-h-[94vh] flex flex-col shadow-2xl overflow-hidden scale-in-center border border-white/10">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 overflow-hidden flex-1 mb-8">
                            ...
                            <div className="lg:col-span-5 space-y-6 overflow-y-auto pr-4 custom-scrollbar font-black uppercase">
                                <div className="bg-gray-50/50 p-7 rounded-[2.5rem] border border-gray-100">
                                    <h4 className="text-[10px] text-gray-300 tracking-widest mb-4">📍 Delivery Info</h4>
                                    <p className="text-xl text-gray-900 mb-1 leading-none">{selectedOrder.customer?.name}</p>
                                    <p className="text-xs text-emerald-600 mb-4 tracking-tighter">📞 {selectedOrder.customer?.phone}</p>
                                    <p className="text-[11px] text-gray-500 tracking-wider italic">{selectedOrder.customer?.address}</p>
                                </div>
                            ...
                            <div className="lg:col-span-7 flex flex-col space-y-6 overflow-hidden">
                                ...
                                <div className="flex-1 bg-gray-50 rounded-[2.5rem] border-2 border-emerald-50 overflow-hidden relative group min-h-[200px]">
                                    {selectedOrder.slipUrl ? (
                                        ...
                                        <div className="h-full flex flex-col items-center justify-center text-gray-300 font-black text-[9px] tracking-widest">Waiting for Slip...</div>
                                    )}
                                </div>
                                ...
                                <div className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm shrink-0">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                                        {Object.entries(ORDER_STATUSES).map(([key, val]) => (
                                            <button 
                                                ...
                                                className={`py-4 rounded-xl text-[10px] font-black uppercase tracking-widest border-2 transition-all shadow-sm
                                            ...
                                            >
                                                {/* 📍 ดึงตรงๆ จาก utils.js (label) จบเลยครับ */}
                                                {val.label}
                                            </button>
                                        ))}
                                    </div>
                                    {selectedOrder.status === 'shipped' && (
                                        <div className="mt-4 flex gap-2 animate-in slide-in-from-top-4">
                                            <input type="text" value={trackingNum} onChange={(e) => setTrackingNum(e.target.value)} className="flex-1 px-4 py-3 bg-gray-50 border border-blue-100 rounded-xl text-xs font-black text
